...
@TeleOp(name="Lupine New Competition TeleOp", group="Iterative Opmode")
public class NewLR24CompTeleOp extends BaseOpMode{
    DcMotor fl;
    DcMotor fr;
    DcMotor bl;
    DcMotor br;
    ...
    public void externalInit() {
        //On init
        fl = hardware.get(DcMotor.class,"fl");
        fr = hardware.get(DcMotor.class,"fr");
        br = hardware.get(DcMotor.class,"br");
        bl = hardware.get(DcMotor.class,"bl");
        depositerArm1 = new Servo("Arm1");
        depositerArm2 = new Servo("Arm2");
        depositerWrist = new Servo("wrist");
        depositerDoor = new Servo("depositerDoor");
        launcher = new Servo("launcher");
        counterRoller = hardware.crservo.get("roll");
        slideMotorL = hardware.get(DcMotor.class,"slideL");
        slideMotorL.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
        slideMotorL.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
        slideMotorL.setTargetPosition(0);
        slideMotorL.setMode(DcMotor.RunMode.RUN_TO_POSITION);
        slideMotorL.setPower(0.8);
        slideMotorR = hardware.get(DcMotor.class,"slideR");
        slideMotorR.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
        slideMotorR.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
        slideMotorR.setTargetPosition(0);
        slideMotorR.setMode(DcMotor.RunMode.RUN_TO_POSITION);
        slideMotorR.setPower(0.8);
        intakeMotor = hardware.get(DcMotor.class,"intake");
        gyro = new Gyro("imu");
        pid = new PID(proportional,integral,derivative);
        depositerArm1.pair(depositerArm2);
...
//        slideMotorL.setTargetPosition(slidePosition);
        ...
        if (driver2.triangle.isTapped() && up) {
            //initiate up
            depositerWrist.setPosition(0.83);
            slidesUp = true;
            time.reset();
            up = false;
            down = true;
        ...
        if (slidesUp) {
            ...
            if (time.seconds() > 1) {
                depositerDoor.setPosition(0);
                depositerArm1.setPosition(0.5);
                depositerArm2.setPosition(0.5);
                fourbarPosition = 0.5;
                depositerWrist.setPosition(0.55);
            }
        }
...
//            initiate down
        ...
        if (slidesDown) {
            if (time.seconds() > 0) {
                ...
                depositerArm1.setPosition(0);
                depositerArm2.setPosition(1);
                fourbarPosition = 0;
                depositerWrist.setPosition(0.5);
            }
            if (time.seconds() > 0.5) {
                slidePosition = 10;
                slidesDown = false;
...
//        if (driver2.dpad_up.isTapped() && driver2.square.isPressed()){
//            fourbarPosition += 0.05;
//            depositerArm1.setPosition(fourbarPosition);
//            depositerArm1.setPosition(fourbarPosition);
//        }
//        if (driver2.dpad_down.isTapped() && driver2.square.isPressed()){
//            fourbarPosition += 0.05;
//            depositerArm1.setPosition(fourbarPosition);
//            depositerArm1.setPosition(fourbarPosition);
//        }
        ...
        addData("heading", gyro.getHeading());
    }
